...
#
# Usage ./abl_mass_qsub_ANIb_contigs.sh path_to_list max_concurrent_submissions output_directory_for_scripts clobberness[keep|clobber]
#
	 	...
	 	if [[ ${counter} -lt ${max_subs} ]]; then
			...
			if [[ -s "${processed}/${project}/${sample}/Contig_check/${sample}_contigs_trimmed.fasta" ]]; then
				if [[ ! -f "${processed}/${sample}/Contig_check/ANI/best_ANIm_contigs_hits_ordered(${sample}_vs_${genus,})" ]]; then
					...
					echo "Going to make ${main_dir}/anibc_${sample}_${start_time}.sh"
					echo -e "#!/bin/bash -l\n" > "${main_dir}/anibc_${sample}_${start_time}.sh"
					echo -e "#$ -o anibc_${sample}.out" >> "${main_dir}/anibc_${sample}_${start_time}.sh"
					echo -e "#$ -e anibc_${sample}.err" >> "${main_dir}/anibc_${sample}_${start_time}.sh"
					echo -e "#$ -N anibc_${sample}"   >> "${main_dir}/anibc_${sample}_${start_time}.sh"
					echo -e "#$ -cwd"  >> "${main_dir}/anibc_${sample}_${start_time}.sh"
					echo -e "#$ -q short.q\n"  >> "${main_dir}/anibc_${sample}_${start_time}.sh"
					echo -e "cd ${shareScript}" >> "${main_dir}/anibc_${sample}_${start_time}.sh"
					echo -e "\"${shareScript}/run_ANIb_contigs.sh\" \"${sample}\" \"${genus}\" \"${species}\" \"${project}\"" >> "${main_dir}/anibc_${sample}_${start_time}.sh"
					echo -e "echo \"$(date)\" > \"${main_dir}/complete/${sample}_anibc_complete.txt\"" >> "${main_dir}/anibc_${sample}_${start_time}.sh"
					cd "${main_dir}"
					#if [[ "${counter}" -lt "${last_index}" ]]; then
						qsub "${main_dir}/anibc_${sample}_${start_time}.sh"
					#else
					#	qsub -sync y "${main_dir}/anibc_${sample}_${start_time}.sh"
					#fi
				# Old data exists, skipping
				else
					echo "${project}/${sample} already has ANI summary"
					echo "$(date)" > "${main_dir}/complete/${sample}_anibc_complete.txt"
				fi
...
printf "%s %s" "abl_mass_qsub_ANIb_contigs.sh has completed" "${global_end_time}" | mail -s "abl_mass_qsub_ANIb_contigs.sh complete" nvx4@cdc.gov

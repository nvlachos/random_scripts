...
if [[ $# -eq 0 ]]; then
	echo "No argument supplied to ./abl_mass_qsub_AQC.sh, exiting"
	exit 1
...
main_dir="${3}/ACS_subs"
if [[ ! -d "${3}/ACS_subs" ]]; then
	mkdir "${3}/ACS_subs"
	mkdir "${3}/ACS_subs/complete"
elif [[ ! -d "${3}/ACS_subs/complete" ]]; then
	mkdir "${3}/ACS_subs/complete"
fi
...
while [ ${counter} -lt ${arr_size} ] ; do
	...
	if [[ "${clobberness}" == "clobber" ]]; then
		...
		if [[ "${processed}/${project}/${sample}/Assembly_Stats_plasmid" ]]; then
			rm -r "${processed}/${project}/${sample}/Assembly_Stats_plasmid"
		fi
	fi
	...
	if [[ -s "${processed}/${project}/${sample}/Assembly/${sample}_scaffolds_trimmed.fasta" ]]; then
		...
		if [[ ${counter} -lt ${max_subs} ]]; then
			...
			if [[ ! -f "${processed}/${project}/${sample}/Assembly_Stats/${sample}_report.txt" ]] || [[ ! -f "${processed}/${project}/${sample}/Assembly_Stats_plasmid/${sample}_report.txt" ]] ; then
				echo  "Index is below max submissions, submitting"
				echo -e "#!/bin/bash -l\n" > "${main_dir}/ACS_${sample}_${start_time}.sh"
				echo -e "#$ -o ACS_${sample}.out" >> "${main_dir}/ACS_${sample}_${start_time}.sh"
				echo -e "#$ -e ACS_${sample}.err" >> "${main_dir}/ACS_${sample}_${start_time}.sh"
				echo -e "#$ -N ACS_${sample}"   >> "${main_dir}/ACS_${sample}_${start_time}.sh"
				echo -e "#$ -cwd"  >> "${main_dir}/ACS_${sample}_${start_time}.sh"
				echo -e "#$ -q short.q\n"  >> "${main_dir}/ACS_${sample}_${start_time}.sh"
				echo -e "cd ${shareScript}" >> "${main_dir}/ACS_${sample}_${start_time}.sh"
				echo -e "\"${shareScript}/run_Assembly_Quality_Check.sh\" \"${sample}\" \"${project}\"" >> "${main_dir}/ACS_${sample}_${start_time}.sh"
				echo -e "echo \"$(date)\" > \"${main_dir}/complete/${sample}_ACS_complete.txt\"" >> "${main_dir}/ACS_${sample}_${start_time}.sh"
				cd "${main_dir}"
				#if [[ "${counter}" -lt "${last_index}" ]]; then
					qsub "${main_dir}/ACS_${sample}_${start_time}.sh"
				#else
				#	qsub -sync y "${main_dir}/ACS_${sample}_${start_time}.sh"
				#fi
			# Old data exists, skipping
			else
				echo "${project}/${sample} already has QUAST summaries"
				echo "$(date)" > "${main_dir}/complete/${sample}_ACS_complete.txt"
			fi

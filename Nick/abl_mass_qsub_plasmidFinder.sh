...
for item in "${arr[@]}"; do
	...
	if [[ -f "${main_dir}/complete/${waiting_sample}_pFn_complete.txt" ]] || [[ ! -s "${processed}/${project}/${waiting_sample}/Assembly/${waiting_sample}_scaffolds_trimmed.fasta" ]]; then
		...
		if [[ -f "${shareScript}/pFn_${waiting_sample}.out" ]]; then
			mv "${shareScript}/pFn_${waiting_sample}.out" ${main_dir}
		fi
		if [[ -f "${shareScript}/pFn_${waiting_sample}.err" ]]; then
 			mv "${shareScript}/pFn_${waiting_sample}.err" ${main_dir}
		fi
	else
		while :
		do

...
while [ ${counter} -lt ${arr_size} ] ; do
	...
	if [ ${counter} -lt ${max_subs} ]; then
		if [[ -s "${processed}/${project}/${sample}/Assembly/contigs.fasta" ]]; then
			...
			echo -e "python \"${shareScript}/removeShortContigs.py\" \"${processed}/${project}/${sample}/Assembly/contigs.fasta\" 500 \"normal_SPAdes\"" >> "${main_dir}/contig_${sample}_${start_time}.sh"

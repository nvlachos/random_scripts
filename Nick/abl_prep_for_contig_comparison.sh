...
while [ ${counter} -lt ${arr_size} ] ; do
	...
	if [ ${counter} -lt ${max_subs} ]; then
		if [[ -s "${processed}/${project}/${sample}/Assembly/contigs.fasta" ]]; then
			...
			echo -e "python \"${shareScript}/removeShortContigs.py\" -i \"${processed}/${project}/${sample}/Assembly/contigs.fasta\" -t 500 -s \"normal_SPAdes\"" >> "${main_dir}/contig_${sample}_${start_time}.sh"
			...
			echo -e "python3 \"${shareScript}/fasta_headers.py\" -i \"${processed}/${project}/${sample}/Contig_check/${sample}_contigs_trimmed_original.fasta\" -o \"${processed}/${project}/${sample}/Contig_check/${sample}_contigs_trimmed.fasta\"" >> "${main_dir}/contig_${sample}_${start_time}.sh"

...
#
# Usage ./run_c-sstar_on_contigs.sh sample_name run_type(g/u for gapped/ungapped) similarity(l/m/h/u/p/o for low(80),medium(95),high(98),ultra-high(99),perfect(100),other(set in config.sh)) run_id (DONT USE-plasmid(optional))
#
# Python/3.5.2
#
...
# Gives the user a brief usage and help section if requested with the -h option argument
	...
	echo "Usage is ./run_c-sstar_on_contigs.sh   sample_name   run-type[g/u](for gapped/ungapped)   similarity[l/m/h/u/p/o](for low/medium/high/ultra-high/perfect as 80/95/98/99/100, other(st in config.sh) run_id (DONT USE-plasmid(optional))"
...
if [[ "${5}" == "--plasmid" ]] || [[ "${5}" == "-p" ]]; then
	if [[ -s "${OUTDATADIR}/Contig_check/${1}_plasmid_contigs_trimmed.fasta" ]]; then
	#if [[ -s "${OUTDATADIR}/plasFlow/Unicycler_assemblies/${1}_uni_assembly/assembly.fasta" ]]; then
		source_assembly="${OUTDATADIR}/Contig_check/${1}_plasmid_contigs_trimmed.fasta"
		OUTDATADIR="${OUTDATADIR}/Contig_check/c-sstar_plasmid"
	...
	if [[ ! -s "${OUTDATADIR}/Contig_check/${1}_contigs_trimmed.fasta" ]]; then
		echo "No Assembly found to run c-sstar with (${OUTDATADIR}/Assembly/${1}_contigs_trimmed.fasta does not exist)"
		exit
	fi
	source_assembly="${OUTDATADIR}/Contig_check/${1}_contigs_trimmed.fasta"
	OUTDATADIR="${OUTDATADIR}/Contig_check/c-sstar"
fi

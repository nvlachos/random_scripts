...
#
# Usage ./run_ANIb.sh sample_name   DB(for looking up reference, just relative path, also is genus)   Species   run_id  list_samples_to_include(optional)
#
# Python/3.5.2
#
...
# Gives the user a brief usage and help section if requested with the -h option argument
	...
	echo "Usage is ./run_ANIb.sh sample_name ani_database(which is also genus) species run_id list_of_samples_to_include(optional)"
	...
	echo "No matching ANI database found for ${2}(genus)" >> "${processed}/${4}/${1}/ANI/best_ANIb_scaffolds_hits_ordered(${1}_vs_${2}).txt"
...
if [ ! -d "$OUTDATADIR/Contig_check/ANI/localANIDB_aniB_scaffolds" ]; then  #create outdir if absent
	echo "Creating $OUTDATADIR/Contig_check/ANI/localANIDB_aniB_scaffolds"
	mkdir -p "$OUTDATADIR/Contig_check/ANI/localANIDB_aniB_scaffolds"
else
	rm -r "$OUTDATADIR/Contig_check/ANI/localANIDB_aniB_scaffolds"
	mkdir -p "$OUTDATADIR/Contig_check/ANI/localANIDB_aniB_scaffolds"
fi
...
cp "${local_DBs}/aniDB/${genus_in,}/"*".fna" "${OUTDATADIR}/Contig_check/ANI/localANIDB_aniB_scaffolds/"
gunzip ${OUTDATADIR}/Contig_check/ANI/localANIDB_aniB_scaffolds/*.gz

#Copies the samples assembly scaffolds to the local ANI db folder
cp "${OUTDATADIR}/Assembly/${1}_scaffolds_trimmed.fasta" "${OUTDATADIR}/Contig_check/ANI/localANIDB_aniB_scaffolds/sample_${2}_${3}.fasta"
	...
	if [[ -f "${5}" ]]; then
		while IFS=read -r var  || [ -n "$var" ]; do
			...
			if [[ "${project}" == "${4}" ]] && [[ "${sample_name}" == "${1}" ]]; then
				...
				cp "${processed}/${project}/${sample_name}/Assembly/${sample_name}_scaffolds_trimmed.fasta" "${OUTDATADIR}/Contig_check/ANI/localANIDB_aniB_scaffolds/temp"
			fi
		done < ${5}
	else
...
#Renames all files in the localANIDB_aniB_scaffolds folder by changing extension from fna to fasta (which pyani needs)
for file in ${OUTDATADIR}/Contig_check/ANI/localANIDB_aniB_scaffolds/*.fna;
do
	fasta_name=$(basename "${file}" .fna)".fasta"
	mv "${file}" "${OUTDATADIR}/Contig_check/ANI/localANIDB_aniB_scaffolds/${fasta_name}"
done
...
sample_count=$(find ${OUTDATADIR}/Contig_check/ANI/localANIDB_aniB_scaffolds/ -type f | wc -l)
...
if [[ ${sample_count} -gt ${max_ani_samples} ]]; then
	sleep 2
	tree=$(head -n 1 "${OUTDATADIR}/Contig_check/ANI/${genus_in}_and_${1}_mashtree_ANIb_scaffolds.dnd")
	echo $tree
	tree=$(echo "${tree}" | tr -d '()')
	echo $tree
	...
	mkdir "${OUTDATADIR}/Contig_check/ANI/localANIDB_aniB_scaffolds_trimmed"
	for sample in ${samples_trimmed[@]};
	do
		filename=$(echo ${sample} | cut -d':' -f1)
		filename="${filename}.fasta"
		echo "Moving ${filename}"
		cp ${OUTDATADIR}/Contig_check/ANI/localANIDB_aniB_scaffolds/${filename} ${OUTDATADIR}/Contig_check/ANI/localANIDB_aniB_scaffolds_trimmed/
	done
	rm -r "${OUTDATADIR}/Contig_check/ANI/localANIDB_aniB_scaffolds_full"
	mv "${OUTDATADIR}/Contig_check/ANI/localANIDB_aniB_scaffolds" "${OUTDATADIR}/Contig_check/ANI/localANIDB_aniB_scaffolds_full"
	mv "${OUTDATADIR}/Contig_check/ANI/localANIDB_aniB_scaffolds_trimmed" "${OUTDATADIR}/Contig_check/ANI/localANIDB_aniB_scaffolds"
...
python -V
#python "${shareScript}/pyani/average_nucleotide_identity.py" -i "${OUTDATADIR}/Contig_check/ANI/localANIDB_aniB_scaffolds" -o "${OUTDATADIR}/Contig_check/ANI/aniB_scaffolds" --write_excel
...
#		echo "Skipping ${i}"
	...
	definition=$(head -1 "${OUTDATADIR}/Contig_check/ANI/localANIDB_aniB_scaffolds/${samples[i]}.fasta")
...
echo -e "${best_percent}%-${best_organism_guess}(${best_file}.fna)\\n$(cat "${OUTDATADIR}/Contig_check/ANI/best_hits_ordered.txt")" > "${OUTDATADIR}/Contig_check/ANI/best_ANIb_scaffolds_hits_ordered(${1}_vs_${genus_in}).txt"

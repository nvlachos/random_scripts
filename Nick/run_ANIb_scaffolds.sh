...
cp "${OUTDATADIR}/Assembly/${1}_scaffolds_trimmed.fasta" "${OUTDATADIR}/Contig_check/ANI/localANIDB/sample_${2}_${3}.fasta"
	...
	if [[ -f "${5}" ]]; then
		while IFS= read -r var; do
			...
			if [[ "${project}" == "${4}" ]] && [[ "${sample_name}" == "${1}" ]]; then
				...
				cp "${processed}/${project}/${sample_name}/Assembly/${sample_name}_scaffolds_trimmed.fasta" "${OUTDATADIR}/Contig_check/ANI/localANIDB/temp"
			fi
		done < ${5}
	else

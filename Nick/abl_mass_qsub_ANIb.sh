...
if [[ $# -eq 0 ]]; then
	echo "No argument supplied to ./abl_mass_qsub_ANI_contigs.sh, exiting"
	exit 1
...
main_dir="${3}/ANIb_scaffolds_subs"
if [[ ! -d "${main_dir}" ]]; then
	mkdir "${main_dir}"
...
while [ ${counter} -lt ${arr_size} ] ; do
	...
	if [[ "${clobbernes}" == "clobber" ]]; then
		rm -r "${processed}/${project}/${sample}/Contig_check/ANI/ANIb_scaffolds/"
		rm -r "${processed}/${project}/${sample}/Contig_check/ANI/localANIDB_ANIb_scaffolds/"
		rm "${processed}/${project}/${sample}/Contig_check/ANI/best_ANIb_scaffolds_hits"*
	fi
	 	...
	 	if [[ ${counter} -lt ${max_subs} ]]; then
			...
			if [[ -s "${processed}/${project}/${sample}/Contig_check/${sample}_contigs_trimmed.fasta" ]]; then
				if [[ ! -f "${processed}/${sample}/Contig_check/ANI/best_ANIm_contigs_hits_ordered(${sample}_vs_${genus,})" ]]; then
					...
					echo "Going to make ${main_dir}/anibs_${sample}_${start_time}.sh"
					echo -e "#!/bin/bash -l\n" > "${main_dir}/anibs_${sample}_${start_time}.sh"
					echo -e "#$ -o anibs_${sample}.out" >> "${main_dir}/anibs_${sample}_${start_time}.sh"
					echo -e "#$ -e anibs_${sample}.err" >> "${main_dir}/anibs_${sample}_${start_time}.sh"
					echo -e "#$ -N anibs_${sample}"   >> "${main_dir}/anibs_${sample}_${start_time}.sh"
					echo -e "#$ -cwd"  >> "${main_dir}/anibs_${sample}_${start_time}.sh"
					echo -e "#$ -q short.q\n"  >> "${main_dir}/anibs_${sample}_${start_time}.sh"
					echo -e "cd ${shareScript}" >> "${main_dir}/anibs_${sample}_${start_time}.sh"
					echo -e "\"${shareScript}/run_ANIb_scaffolds.sh\" \"${sample}\" \"${genus}\" \"${species}\" \"${project}\"" >> "${main_dir}/anibs_${sample}_${start_time}.sh"
					echo -e "echo \"$(date)\" > \"${main_dir}/complete/${sample}_anibs_complete.txt\"" >> "${main_dir}/anibs_${sample}_${start_time}.sh"
					cd "${main_dir}"
					#if [[ "${counter}" -lt "${last_index}" ]]; then
						qsub "${main_dir}/anibs_${sample}_${start_time}.sh"
					#else
					#	qsub -sync y "${main_dir}/anibs_${sample}_${start_time}.sh"
					#fi
				# Old data exists, skipping
				else
					echo "${project}/${sample} already has ANI summary"
					echo "$(date)" > "${main_dir}/complete/${sample}_anibs_complete.txt"
				fi
	...
	# Counter is above limit, wait until "slot" opens up"
		...
		do
			...
			if [[ -f "${main_dir}/complete/${waiting_sample}_anibs_complete.txt" ]]; then
...
for item in "${arr[@]}"; do
	...
	if [[ -f "${main_dir}/complete/${waiting_sample}_anibs_complete.txt" ]]; then
		...
		if [[ -f "${shareScript}/anibs_${sample}.out" ]]; then
			mv "${shareScript}/anibs_${sample}.out" ${main_dir}
		fi
		if [[ -f "${shareScript}/anibs_${sample}.err" ]]; then
			mv "${shareScript}/anibs_${sample}.err" ${main_dir}
		fi
	else

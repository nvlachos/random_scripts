...
#
# Usage ./abl_masss_qsub_kraka.sh path_to_list max_concurrent_submission output_directory_for_scripts clobberness[keep|clobber]
#
...
# Shows a brief uasge/help section if -h option used as first argument
	...
	echo "Usage is ./abl_mass_qsub_kraka.sh path_to_list_file(single sample ID per line, e.g. B8VHY/1700128 (it must include project id also)) max_concurrent_submissions output_directory_for_scripts"
	exit 1
elif [[ ! -f "${1}" ]]; then
		...
		if [[ ${counter} -lt ${max_subs} ]]; then
			...
			if [[ ! -f "${processed}/${project}/${sample}/kraken/postAssembly/${sample}_kraken_summary_assembled_BP.txt" ]]; then
				if [[ ! -f "${processed}/${project}/${sample}/kraken/postAssembly/${sample}_kraken_summary_assembled_BP_data.txt" ]]; then
					rm "${processed}/${project}/${sample}/kraken/postAssembly/${sample}_kraken_summary_assembled_BP_data.txt"
				fi
				...
				echo -e "cd ${shareScript}" >> "${main_dir}/kraka_${sample}_${start_time}.sh"
				echo -e "\"${shareScript}/run_kraken.sh\" \"${sample}\" post assembled \"${project}\"" >> "${main_dir}/kraka_${sample}_${start_time}.sh"
				echo -e "echo \"$(date)\" > \"${main_dir}/complete/${sample}_kraka_complete.txt\"" >> "${main_dir}/kraka_${sample}_${start_time}.sh"
				cd "${main_dir}"
				#if [[ "${counter}" -lt "${last_index}" ]]; then
					qsub "${main_dir}/kraka_${sample}_${start_time}.sh"
				#else
				#	qsub -sync y "${main_dir}/kraka_${sample}_${start_time}.sh"
				#fi
			# Old data exists, skipping
			else
				echo "${project}/${sample} already has kraken weighted summary"
				echo "$(date)" > "${main_dir}/complete/${sample}_kraka_complete.txt"
			fi
...
printf "%s %s" "abl_mass_qsub_kraka.sh has completed" "${global_end_time}" | mail -s "abl_mass_qsub_kraka.sh complete" nvx4@cdc.gov

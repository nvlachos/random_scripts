...
#
# Output location: default_config.sh_output_location/run_ID/sample_name/ANI_2/
#
...
# Gives the user a brief usage and help section if requested with the -h option argument
	...
	echo "Output is saved to in ${processed}/sample_name/ANI_2"
	exit 0
elif [ -z "$2" ]; then
	...
	if [ ! -d "${processed}/${4}/${1}/ANI_2" ]; then  #create outdir if absent
		echo "${processed}/${4}/${1}/ANI_2"
		mkdir -p "${processed}/${4}/${1}/ANI_2"
	fi
	# Write non-results to a file in ANI folder
	echo "No matching ANI database found for ${2}(genus)" >> "${processed}/${4}/${1}/ANI_2/best_ANI_hits_ordered(${1}_vs_${2}).txt"
...
if [ ! -d "$OUTDATADIR/ANI_2" ]; then
	echo "Creating $OUTDATADIR/ANI_2"
	mkdir -p "$OUTDATADIR/ANI_2"
fi
...
if [ ! -d "$OUTDATADIR/ANI_2/localANIDB" ]; then  #create outdir if absent
	echo "Creating $OUTDATADIR/ANI_2/localANIDB"
	mkdir -p "$OUTDATADIR/ANI_2/localANIDB"
else
	rm -r "$OUTDATADIR/ANI_2/localANIDB"
	mkdir -p "$OUTDATADIR/ANI_2/localANIDB"
fi

# Checks to see if the local DB ANI folder already exists and creates it if not. This is used to store a local copy of all samples in DB to be compared to (as to not disturb the source DB)
if [ ! -d "$OUTDATADIR/ANI_2/temp" ]; then  #create outdir if absent
	echo "Creating $OUTDATADIR/ANI_2/temp"
	mkdir -p "$OUTDATADIR/ANI_2/temp"
else
	rm -r "$OUTDATADIR/ANI_2/temp"
	mkdir -p "$OUTDATADIR/ANI_2/temp"
fi
...
cp "${OUTDATADIR}/Assembly/${1}_scaffolds_trimmed.fasta" "${OUTDATADIR}/ANI_2/localANIDB/sample_${2}_${3}.fasta"
	...
	if [[ -f "${5}" ]]; then
		while IFS=read -r var  || [ -n "$var" ]; do
			...
			if [[ "${project}" == "${4}" ]] && [[ "${sample_name}" == "${1}" ]]; then
				...
				cp "${processed}/${project}/${sample_name}/Assembly/${sample_name}_scaffolds_trimmed.fasta" "${OUTDATADIR}/ANI_2/localANIDB"
			fi
		done < ${5}
	else
...
for file in ${OUTDATADIR}/ANI_2/localANIDB/*.fna;
do
	fasta_name=$(basename "${file}" .fna)".fasta"
	mv "${file}" "${OUTDATADIR}/ANI_2/localANIDB/${fasta_name}"
done

# Mashtree trimming to reduce run time for ANI
owd=$(pwd)
cd ${OUTDATADIR}/ANI_2/localANIDB/
mashtree --numcpus ${procs} *.fasta --tempdir ${OUTDATADIR}/ANI_2/temp > ${OUTDATADIR}/ANI_2/"${genus_in}_and_${1}_mashtree.dnd";

# Get total number of isolates compared in tree
sample_count=$(find ${OUTDATADIR}/ANI_2/localANIDB/ -type f | wc -l)
...
if [[ ${sample_count} -gt ${max_ani_samples} ]]; then
	sleep 2
	tree=$(head -n 1 "${OUTDATADIR}/ANI_2/${genus_in}_and_${1}_mashtree.dnd")
	echo $tree
	tree=$(echo "${tree}" | tr -d '()')
	echo $tree
	...
	mkdir "${OUTDATADIR}/ANI_2/localANIDB_trimmed"
	for sample in ${samples_trimmed[@]};
	do
		filename=$(echo ${sample} | cut -d':' -f1)
		filename="${filename}.fasta"
		echo "Moving ${filename}"
		cp ${OUTDATADIR}/ANI_2/localANIDB/${filename} ${OUTDATADIR}/ANI_2/localANIDB_trimmed/
	done
	if [[ -d "${OUTDATADIR}/ANI_2/localANIDB_full" ]]; then
		rm -r "${OUTDATADIR}/ANI_2/localANIDB_full"
	fi
	mv "${OUTDATADIR}/ANI_2/localANIDB" "${OUTDATADIR}/ANI_2/localANIDB_full"
	mv "${OUTDATADIR}/ANI_2/localANIDB_trimmed" "${OUTDATADIR}/ANI_2/localANIDB"
...
if [ -d "${OUTDATADIR}/ANI_2/aniM" ]; then
	echo "Removing old ANIm results in ${OUTDATADIR}/ANI_2/aniM"
	rm -r "${OUTDATADIR}/ANI_2/aniM"
fi
...
if [[ -d "${OUTDATADIR}/ANI_2/temp" ]]; then
	rm -r "${OUTDATADIR}/ANI_2/temp"
fi

#Calls pyani on local db folder
python -V
#python "${shareScript}/pyani/average_nucleotide_identity.py" -i "${OUTDATADIR}/ANI_2/localANIDB" -o "${OUTDATADIR}/ANI_2/aniM" --write_excel
average_nucleotide_identity.py -i "${OUTDATADIR}/ANI_2/localANIDB" -o "${OUTDATADIR}/ANI_2/aniM" --write_excel
...
done < "${OUTDATADIR}/ANI_2/aniM/ANIm_percentage_identity.tab"
...
if [[ -s "${OUTDATADIR}/ANI_2/aniM/ANIm_percentage_identity.tab" ]]; then
	firstline=$(head -n 1 "${OUTDATADIR}/ANI_2/aniM/ANIm_percentage_identity.tab")
else
	echo "No "${OUTDATADIR}/ANI_2/aniM/ANIm_percentage_identity.tab" file, exiting"
	exit 1
fi
...
#		echo "Skipping ${i}"
	...
	definition=$(head -1 "${OUTDATADIR}/ANI_2/localANIDB/${samples[i]}.fasta")
	# Prints all matching samples to file (Except the self comparison) by line as percent_match  sample_name  fasta_header
	echo "${percents[i+1]} ${samples[i]} ${definition}" >> "${OUTDATADIR}/ANI_2/best_hits.txt"
done

#Sorts the list in the file based on %id (best to worst)
sort -nr -t' ' -k1 -o "${OUTDATADIR}/ANI_2/best_hits_ordered.txt" "${OUTDATADIR}/ANI_2/best_hits.txt"
#Extracts the first line of the file (best hit)
best=$(head -n 1 "${OUTDATADIR}/ANI_2/best_hits_ordered.txt")
...
echo -e "${best_percent}%-${best_organism_guess}(${best_file}.fna)\\n$(cat "${OUTDATADIR}/ANI_2/best_hits_ordered.txt")" > "${OUTDATADIR}/ANI_2/best_ANI_hits_ordered(${1}_vs_${genus_in}).txt"

#Removes the transient hit files
if [ -s "${OUTDATADIR}/ANI_2/best_hits.txt" ]; then
	rm "${OUTDATADIR}/ANI_2/best_hits.txt"
#	echo "1"
fi
if [ -s "${OUTDATADIR}/ANI_2/best_hits_ordered.txt" ]; then
	rm "${OUTDATADIR}/ANI_2/best_hits_ordered.txt"
#	echo "2"
fi

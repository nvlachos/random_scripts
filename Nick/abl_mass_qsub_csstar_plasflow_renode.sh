...
while [ ${counter} -lt ${arr_size} ] ; do
	...
	if [[ -f "${processed}/${project}/${sample}/c-sstar_plasFlow/${sample}.${ResGANNCBI_srst2_filename}.gapped_40_sstar_summary.txt" ]]; then
		id=$(head -n1 "${processed}/${project}/${sample}/c-sstar_plasFlow/${sample}.${ResGANNCBI_srst2_filename}.gapped_40_sstar_summary.txt" | cut -d'	' -f6)
		if [[ "${id}" != ${sample} ]]; then
			renode="true"
		fi
	fi
	if [[ "${clobberness}" = "clobber" ]] || [[ "${renode}" = "true" ]]; then

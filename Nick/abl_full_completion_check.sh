...
# Check to see if all samples have the normal 0608 AR csstar output
	...
	if [[ -s "${processed}/${project}/${sample_name}/FASTQs/${sample_name}_R1_001.paired.fq.gz" ]]; then
		FQTZR1="Found"
	else
		FQTZR1="NOT FOUND"
	fi
	...
	if [[ -s "${processed}/${project}/${sample_name}/16s/${sample_name}_16s_blast_id.txt" ]]; then
		...
		16s="${best16s}:${largest16s}"
	else
		16s="NOT_FOUND"
	fi

	if [[ -s "${processed}/${project}/${sample_name}/prokka/${sample_name}_PROKKA.gbf" ]]; then
		prokka="Found"
	else
		prokka="NOT_FOUND"
	fi

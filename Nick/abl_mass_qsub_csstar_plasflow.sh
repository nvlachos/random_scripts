...
while [ ${counter} -lt ${arr_size} ] ; do
	...
	if [[ -s "${processed}/${project}/${sample}/plasFlow/Unicycler_assemblies/${sample}_assembly.fasta" ]]; then
		#echo "Test"
		...
		# Counter is above max number of submissions
			...
			do
				...
				if [[ -f "${main_dir}/complete/${waiting_sample}_csstarp_complete.txt" ]] || [[ ! -s "${processed}/${project}/${waiting_sample}/plasFlow/Unicycler_assemblies/${sample}_assembly.fasta" ]]; then
					# Check if old data exists, skip if so
					if [[ ! -f "${processed}/${project}/${sample}/c-sstar_plasFlow/${sample}.${resGANNOT_srst2_filename}.gapped_40_sstar_summary.txt" ]]; then
						echo  "Index is below max submissions, submitting"
						echo -e "#!/bin/bash -l\n" > "${main_dir}/csstp_${sample}_${start_time}.sh"
...
for item in "${arr[@]}"; do
	...
	if [[ -f "${main_dir}/complete/${waiting_sample}_csstarp_complete.txt" ]] || [[ ! -s "${processed}/${project}/${waiting_sample}/plasFlow/Unicycler_assemblies/${waiting_sample}_assembly.fasta" ]]; then

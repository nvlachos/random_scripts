...
while [ ${counter} -lt ${arr_size} ] ; do
	...
	if [[ -f "${processed}/${project}/${sample}/c-sstar/${sample}.${ResGANNCBI_srst2_filename}.gapped_${simnum}_sstar_summary.txt" ]]; then
		id=$(head -n1 "${processed}/${project}/${sample}/c-sstar/${sample}.${ResGANNCBI_srst2_filename}.gapped_${simnum}_sstar_summary.txt" | cut -d'	' -f6)
		if [[ "${id}" = "node" ]]; then
			node="true"
		fi
	fi
	if [[ "${clobberness}" = "clobber" ]] || [[ "${node}" = "true" ]]; then

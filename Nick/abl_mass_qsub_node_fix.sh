...
#
# Usage ./abl_mass_qsub_node_fix.sh path_to_list max_concurrent_submission output_directory_for_scripts clobberness[keep|clobber]
#
...
while [ ${counter} -lt ${arr_size} ] ; do
	...
	if [[ "${clobberness}" == "clobber" ]]; then
		if [[ -s "${processed}/${project}/${sample}/Assembly/${sample}_scaffolds_trimmed.fasta" ]]; then
			...
			elif [[ "${header}" = ">${sample}" ]]; then
				:
			else
				echo "Unknown Contig name(${header}), exiting"
			fi
		fi
	fi
	# echo ${counter}

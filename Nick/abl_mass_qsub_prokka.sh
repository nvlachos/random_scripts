	...
	if [[ -s "${processed}/${project}/${sample}/Assembly/${sample}_scaffolds_trimmed.fasta" ]]; then
		...
		if [[ ${counter} -lt ${max_subs} ]]; then
			...
			if [[ ! -f "${processed}/${project}/${sample}/prokka/${sample}_PROKKA.gbf" ]]; then
				...
				echo -e "python3 ${shareScript}/fasta_headers.py -i ${processed}/${project}/${sample}/Assembly/${sample}_scaffolds_trimmed.fasta_backup -o ${processed}/${project}/${sample}/Assembly/${sample}_scaffolds_trimmed.fasta --reverse" >> "${main_dir}/prok_${sample}_${start_time}.sh"

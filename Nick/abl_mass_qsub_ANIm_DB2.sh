	...
	if [[ -s "${processed}/${project}/${sample}/${sample}.tax" ]]; then
	 	...
	 	if [[ ${counter} -lt ${max_subs} ]]; then
			...
			if [[ -s "${processed}/${project}/${sample}/Assembly/${sample}_scaffolds_trimmed.fasta" ]]; then
				if [[ ! -f "${processed}/${sample}/ANI/best_anim_hits_ordered(${sample}_vs_${genus,})" ]]; then
					...
					echo "Going to make ${main_dir}/anim2_${sample}_${start_time}.sh"
					echo -e "#!/bin/bash -l\n" > "${main_dir}/anim2_${sample}_${start_time}.sh"
					echo -e "#$ -o anim2_${sample}.out" >> "${main_dir}/anim2_${sample}_${start_time}.sh"
					echo -e "#$ -e anim2_${sample}.err" >> "${main_dir}/anim2_${sample}_${start_time}.sh"
					echo -e "#$ -N anim2_${sample}"   >> "${main_dir}/anim2_${sample}_${start_time}.sh"
					echo -e "#$ -cwd"  >> "${main_dir}/anim2_${sample}_${start_time}.sh"
					echo -e "#$ -q short.q\n"  >> "${main_dir}/anim2_${sample}_${start_time}.sh"
					echo -e "cd ${shareScript}" >> "${main_dir}/anim2_${sample}_${start_time}.sh"
					echo -e "\"${shareScript}/run_ANIm_DB2.sh\" \"${sample}\" \"${genus}\" \"${species}\" \"${project}\"" >> "${main_dir}/anim2_${sample}_${start_time}.sh"
					echo -e "\"${shareScript}/determine_taxID_aniDB2.sh\" \"${sample}\" \"${project}\"" >> "${main_dir}/anim2_${sample}_${start_time}.sh"
					echo -e "echo \"$(date)\" > \"${main_dir}/complete/${sample}_anim2_complete.txt\"" >> "${main_dir}/anim2_${sample}_${start_time}.sh"
					cd "${main_dir}"
					qsub "${main_dir}/anim2_${sample}_${start_time}.sh"
				# Old data exists, skipping
				else
					echo "${project}/${sample} already has ANI summary"
					echo "$(date)" > "${main_dir}/complete/${sample}_anim2_complete.txt"
				fi
		...
		# Counter is above limit, wait until "slot" opens up"
			...
			do
				...
				if [[ -f "${main_dir}/complete/${waiting_sample}_anim2_complete.txt" ]]; then
...
for item in "${arr[@]}"; do
	...
	if [[ -f "${main_dir}/complete/${waiting_sample}_anim2_complete.txt" ]]; then
		...
		if [[ -f "${shareScript}/anim2_${waiting_sample}.out" ]]; then
			mv "${shareScript}/anim2_${waiting_sample}.out" ${main_dir}
		fi
		if [[ -f "${shareScript}/anim2_${waiting_sample}.err" ]]; then
			mv "${shareScript}/anim2_${waiting_sample}.err" ${main_dir}
		fi
	else
		...
		do
				...
				if [[ -f "${main_dir}/complete/${waiting_sample}_anim2_complete.txt" ]]; then
					echo "${item} is complete"
					if [[ -f "${shareScript}/anim2_$waiting_{sample}.out" ]]; then
						mv "${shareScript}/anim2_${waiting_sample}.out" ${main_dir}
					fi
					if [[ -f "${shareScript}/anim2_${waiting_sample}.err" ]]; then
						mv "${shareScript}/anim2_${waiting_sample}.err" ${main_dir}

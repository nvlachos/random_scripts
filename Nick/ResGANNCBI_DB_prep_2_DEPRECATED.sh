...
#
# Usage ./ResGANNOT_DB_prep_2.sh path_to_ResGANNOT.fasta	path_to_NCBI.fasta	path_to_output_dir
#
...
if [[ $# -eq 0 ]]; then
	...
	# Gives the user a brief usage and help section if requested with the -h option argument
		...
		echo "Usage is ./ResGANNCBI_DB_prep_2.sh path_to_ResGANNOT.fasta	path_to_NCBI_fasta	(path_to_output_dir)"
		echo "Converts the ResGANNOT fasta and NCBI fasta files into a single fasta file to a single srst2 DB for use with csstar and srst2"
...
if [[ "${non_duplicated}" != "true" ]]; then
	...
	python "${shareScript}/ResGANNOT_Combiner_Non_Duplicate_Exe.py" "${ResGANNOT_source}" "${ARGANNOT_source}" "${NCBI_source}" "${ResGANNCBI_source}" "${DATADIR}/ResGANNCBI_${today}_Copies.fasta" "RGN" "NCBI"

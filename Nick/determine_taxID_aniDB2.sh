...
Check_source() {
	...
	if [[ "${start_at}" -le 1 ]]; then
		for f in ${processed}/${project}/${sample}/ANI_2/*; do
...
do_ANI() {
	source="ANI"
	#echo "${source}"
	if [[ -f "${processed}/${project}/${sample}/ANI_2/best_ANI_hits_ordered(${sample}_vs_All).txt" ]]; then
		source_file="${processed}/${project}/${sample}/ANI_2/best_ANI_hits_ordered(${sample}_vs_All).txt"
	else
		source_file=$(ls -t "${processed}/${project}/${sample}/ANI_2/best_ANI_hits_ordered"* | head -n 1)
	fi
...
echo -e "(${source})-${confidence_index}%-${source_file}\nD:	${Domain}\nP:	${Phylum}\nC:	${Class}\nO:	${Order}\nF:	${Family}\nG:	${Genus}\ns:	${species}\n" > "${processed}/${project}/${sample}/${sample}_aniDB2.tax"

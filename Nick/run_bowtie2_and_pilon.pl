...
### Use full path for DB (pacbio assembly) and miseq reads folder
...
foreach my $file (@files){
	...
	#attempt different naming scheme
		...
		print $file_name
		warn "Input file does not contain '_R1' or '_R2' in name: $file";
	}
}
...
foreach my $name (sort keys %paired_files){
    ...
    system("pilon --genome $DB --frags $path$name.sort.bam --mindepth 0.6 --minqual 30 --minmq 30 --fix all --changes --vcf --output $path$name.$DB_shortname.pilon");
}
